...
class Config:
    def __init__(self):        
        self.download_url = get_env('RTA_URL', 'https://ti.arc.nasa.gov/m/project/prognostic-repository/mill.zip')
        self.download_file = get_env('RTA_FILE', 'nasa_mill.txt')
        self.download_file_1 = get_env('RTA_FILE_1', 'nasa_mill_1.txt')
        self.result_file = get_env('RTA_RESULT', 'nasa_mill.influxdb')
        self.result_file_1 = get_env('RTA_RESULT', 'nasa_mill_1.influxdb')
        ...
        self.tag_values = get_env('RTA_TAG_VALUES', 'case').split(self.split_symbol)
        self.tag_keys_1 = get_env('RTA_TAG_KEYS', 'metric').split(' ')
        self.tag_values_1 = get_env('RTA_TAG_VALUES', 'current').split(self.split_symbol)
        self.field_keys = get_env('RTA_FIELD_KEYS', 'case;run;VB;time;DOC;feed;material').split(self.split_symbol)
        self.field_values = get_env('RTA_FIELD_VALUES', '$1 $2 $3 $4 $5 $6 $7').split(' ')
        self.field_keys_1 = get_env('RTA_FIELD_KEYS_1', 'smcAC;smcDC;vib_table;vib_spindle;AE_table;AE_spindle').split(self.split_symbol)
        self.field_values_1 = get_env('RTA_FIELD_VALUES_1', '$1 $2 $3 $4 $5 $6').split(' ')
        ...
        self.measurement = get_env('RTA_MEASUREMENT', 'mill')
...
class Simulator:
    ...
    def preprocess(self):
        def getstamp(y=2012,m=2,d=2,n=0,h=0,shif=0.0):
            the_date = datetime.datetime(y,m,d)
            result_date = the_date + datetime.timedelta(days=-n) + datetime.timedelta(hours=(h*24))
            d = result_date.strftime('%a %b %d %H:%M:%S %Y')
            timestamp = int((time.mktime(time.strptime(d))+shif)*1000000000)
            return timestamp
        data = sio.loadmat('./mill.mat')
        data = data['mill']
        index = data.shape[1]#167
        with open(self.config.download_file, 'w') as writer:
            for ind in range(index):
                case = data[0][ind]
                casenum = case['case'][0][0]
                runnum = case['run'][0][0]
                vb = case['VB'][0][0]
                timeused = case['time'][0][0]
                stamp = getstamp(2018,7,3,int(casenum),timeused/180.0,0.0)
                doc = case['DOC'][0][0]
                feed = case['feed'][0][0]
                material = case['material'][0][0]
                line_data = '{casenum} {runnum} {vb} {timeused} {doc} {feed} {material} {stamp}\n'.format(casenum=str(casenum),runnum=str(runnum),vb=str(vb),timeused=str(timeused),doc=str(doc),feed=str(feed),material=str(material),stamp=str(stamp))
                writer.write(line_data)
                # manipulating simulation data in each experiments/case
                index_sim = case['smcAC'].shape[0]
                with open(self.config.download_file_1, 'a+') as writer_sim:
                    for ind_sim in range(index_sim):
                        smcAC = case['smcAC'][ind_sim][0]
                        smcDC = case['smcDC'][ind_sim][0]
                        vib_table = case['vib_table'][ind_sim][0]
                        vib_spindle = case['vib_spindle'][ind_sim][0]
                        ae_table = case['AE_table'][ind_sim][0]
                        ae_spindle = case['AE_spindle'][ind_sim][0]
                        stamp_sim = getstamp(2018,7,3,int(casenum),timeused/180.0,ind_sim/360000.0)
                        line_data_sim = '{smcAC} {smcDC} {vib_table} {vib_spindle} {ae_table} {ae_spindle} {stamp_sim}\n'.format(smcAC=str(smcAC),smcDC=str(smcDC),vib_table=str(vib_table),vib_spindle=str(vib_spindle),ae_table=str(ae_table),ae_spindle=str(ae_spindle),stamp_sim=str(stamp_sim))
                        writer_sim.write(line_data_sim)
                print('%d/%d'%(ind,index))

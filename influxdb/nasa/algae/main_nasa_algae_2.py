...
class Config:
    def __init__(self):        
        ...
        self.download_file = get_env('RTA_FILE', 'nasa_algae_2.txt')
        self.result_file = get_env('RTA_RESULT', 'nasa_algae_2.influxdb')
        ...
        self.field_keys = get_env('RTA_FIELD_KEYS', 'delta_pH;delta_nitrate;delta_phosphate;delta_density').split(self.split_symbol)
        self.field_values = get_env('RTA_FIELD_VALUES', '$1 $2 $3 $4').split(' ')
...
class Simulator:
    ...
    def preprocess(self):
        ...
        def getstamp(y=2012,m=2,d=2,n=0,h=0.67):
            ...
            return timestamp
        ind = algae_ts[0][0]['delta_pH'][0][0]['data'].shape[1]
        with open(self.config.download_file, 'w') as writer:
            for index in range(0,ind):
                for i in range(0,r):
                    for j in range(0,c):
                        algae = algae_ts[i][j]# each raceway
                        delta_pH = algae['delta_pH']
                        data_pH = delta_pH[0][0]['data']
                        pH = data_pH[0][index]
                        delta_nitrate = algae['delta_nitrate']
                        data_nitrate = delta_nitrate[0][0]['data']
                        nitrate = data_nitrate[0][index]
                        delta_phosphate = algae['delta_phosphate']
                        data_phosphate = delta_phosphate[0][0]['data']
                        phosphate = data_phosphate[0][index]
                        delta_density = algae['delta_density']
                        data_density = delta_density[0][0]['data']
                        density = data_density[0][index]
                        time_stam = delta_pH[0][0]['time_num']
                        stamp = time_stam[index][0]
                        timestamp = getstamp(2012,2,2,int(stamp),float(0.671))
                        line_data = '{pH} {nitrate} {phosphate} {density} {stamp}\n'.format(pH=str(pH),nitrate=str(nitrate),phosphate=str(phosphate),density=str(density),stamp=str(timestamp))
                        # print(line_data)
                # break

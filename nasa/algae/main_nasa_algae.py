...
class Config:
    def __init__(self):        
        ...
        self.download_file = get_env('RTA_FILE', 'nasa_algae.txt')
        self.result_file = get_env('RTA_RESULT', 'nasa_algae.influxdb')
        ...
        self.field_keys = get_env('RTA_FIELD_KEYS', 'irradiance;temperature').split(self.split_symbol)
        self.field_values = get_env('RTA_FIELD_VALUES', '$1 $2').split(' ')
...
class Simulator:
    ...
    def preprocess(self):
        ...
        r, c = algae_ts.shape
        def getstamp(y=2012,m=2,d=2,n=0,h=0):
            ...
            return timestamp
        ind = algae_ts[0][0]['irradiance'][0][0]['data'].shape[0]
        with open(self.config.download_file, 'w') as writer:
            for index in range(0,ind):
                for i in range(0,r):
                    for j in range(0,c):
                        algae = algae_ts[i][j]# each raceway
                        irradiance = algae['irradiance']
                        temperature = algae['temperature']
                        data_irr = irradiance[0][0]['data']
                        time_stam = irradiance[0][0]['time_num']
                        data_temp = temperature[0][0]['data']
                        irr = data_irr[index][0]
                        temp = data_temp[index][0]
                        stamp = time_stam[index][0]
                        timestamp = getstamp(2012,2,2,int(stamp),stamp-int(stamp))
                        line_data = '{irr} {temp} {stamp}\n'.format(irr=str(irr),temp=str(temp),stamp=str(timestamp))
